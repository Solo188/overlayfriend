...
// =============================================================================
// === НАСТРОЙКА ЦВЕТА И ЯРКОСТИ СВЕТА =========================================
// =============================================================================
//   LIGHT_COL — цвет рассеянного света. Чем меньше значения RGB, тем
//   холоднее и темнее свет. Текущий цвет: мягкий тёплый серый.
//   Примеры:
//     vec3(1.0, 0.98, 0.95)  — яркий тёплый белый (оригинал)
//     vec3(0.7, 0.68, 0.65)  — мягкий приглушённый (текущий)
//     vec3(0.5, 0.5, 0.55)   — холодный полумрак
// =============================================================================
const vec3 LIGHT_COL  = vec3(0.7, 0.68, 0.65);
...
void main() {
    ...
    if (u_hasTexture == 1) {
        ...
        if (texColor.a < 0.1) discard;   // === ПОРОГ ОТСЕЧЕНИЯ ТЕКСТУРЫ ===
                                          // 0.1 = мягкие края без "мыла"
                                          // Увеличить до 0.3 — жёсткое отсечение
                                          // Уменьшить до 0.05 — почти нет отсечения
        albedo = texColor.rgb;
        alpha  = texColor.a * u_diffuse.a;
    }
    // No texture: use diffuse.a as-is (shadow mats alpha=0.3 by design).

    // === ПОРОГ ОТСЕЧЕНИЯ ИТОГОВОЙ ПРОЗРАЧНОСТИ ===
    // Пиксель выбрасывается если итоговый alpha слишком мал.
    // 0.1 — точные края без "мыльного" ореола по контуру.
    // Увеличить до 0.2 — жёстче, убирает полупрозрачные пиксели.
    // Уменьшить до 0.05 — мягче, сохраняет тонкие полупрозрачные края.
    if (alpha * u_globalAlpha < 0.1) discard;

    vec3  N     = normalize(v_normal);
    float NdotL = max(dot(N, LIGHT_DIR), 0.0);

    // ── MMD standard lighting ─────────────────────────────────────────────

    // =============================================================================
    // === НАСТРОЙКА ЯРКОСТИ СВЕТА =================================================
    // =============================================================================
    //   Число 0.35 — это сила направленного (рассеянного) света.
    //
    //   КАК ПОДКРУТИТЬ:
    //     Уменьшить 0.35  →  модель становится ТЕМНЕЕ (меньше света падает)
    //     Увеличить 0.35  →  модель становится СВЕТЛЕЕ (больше света падает)
    //
    //   Рекомендуемый диапазон: от 0.2 (очень тёмно) до 0.8 (очень ярко).
    //   При значениях выше 0.8 появляются засветы на коже и одежде.
    //
    //   ambient (u_ambient) — это базовый "фон" тени, он всегда присутствует
    //   независимо от направления света. Менять его не нужно.
    // =============================================================================
    vec3 litColor = albedo * u_ambient                       // базовый фон (тень)
                  + albedo * LIGHT_COL * NdotL * 0.35;      // направленный свет
    ...
    // alpha         — непрозрачность из материала/текстуры (0.0 – 1.0)
    // u_globalAlpha — глобальный ползунок Opacity из интерфейса
    // Перемножение даёт плавное угасание всей модели одновременно.
...
void MMDRenderer::render(float dt) {
    ...
    // =========================================================================
    // === НАСТРОЙКА КАМЕРЫ И ДИСТАНЦИИ ========================================
    // =========================================================================
    //   glm::perspective — проекция камеры:
    //     30.0f  — угол обзора (FOV). Меньше = телефото (меньше искажений лица).
    //     1.0f   — ближняя плоскость отсечения. НЕ МЕНЯТЬ — критично для Z-буфера.
    //     5000.0f — дальняя плоскость. Можно увеличить если что-то обрезается вдали.
    //
    //   glm::lookAt — позиция и направление камеры:
    //     Первый vec3  — позиция камеры:  (X=0, Y=11.5f, Z=50.0f)
    //       Z=50.0f — ОТДАЛЕНИЕ камеры от модели.
    //         Если модель не влазит целиком → увеличить (55.0f, 60.0f ...)
    //         Если модель слишком мелкая    → уменьшить (40.0f, 35.0f ...)
    //     Второй vec3 — точка взгляда:    (X=0, Y=11.5f, Z=0)
    //       Y=11.5f — ВЫСОТА взгляда (уровень пояса/груди модели при FOV=30).
    //         Увеличить Y → камера смотрит выше (голова и лицо в центре)
    //         Уменьшить Y → камера смотрит ниже (ноги и тело в центре)
    // =========================================================================
    glm::mat4 proj = glm::perspective(glm::radians(30.0f), aspect, 1.0f, 5000.0f);
    glm::mat4 view = glm::lookAt(
        glm::vec3(0.0f, 11.0f, 55.0f),   // === ПОЗИЦИЯ КАМЕРЫ ===
                                           // Z=55.0f — дистанция (дальше = вся модель видна)
                                           // Модель не влазит? → 60.0f, 65.0f ...
                                           // Модель слишком мелкая? → 45.0f, 40.0f ...
                                           // Y=11.0f — высота камеры (уровень груди/пояса)
        glm::vec3(0.0f, 11.0f,  0.0f),   // === ТОЧКА ВЗГЛЯДА ===
                                           // Y должен совпадать с Y позиции камеры.
                                           // Увеличить Y → смотрим выше (голова в центре).
                                           // Уменьшить Y → смотрим ниже (ноги в центре).
        glm::vec3(0.0f,  1.0f,  0.0f));  // вектор "вверх" — не менять
...
void MMDRenderer::drawModel() {
    ...
    for (size_t li = 0; li < loopCount; ++li) {
        ...
        // =====================================================================
        // === ФИКС КАШИ НА ЛИЦЕ + СТЕКЛЯННЫХ РУК (OPACITY) ===================
        // =====================================================================
        //
        //   ПРОБЛЕМА 1 — Рот и глаза "проваливаются" в кожу (Z-fighting):
        //     Решается через Polygon Offset — небольшой сдвиг Z-значения
        //     ближе к камере, чтобы детали всегда рисовались поверх кожи.
        //
        //   ПРОБЛЕМА 2 — Руки "стеклянные" при уменьшении ползунка Opacity:
        //     Причина — glDepthMask(GL_FALSE) в прозрачном пассе отключает
        //     запись в Z-буфер, и рука видна сквозь саму себя.
        //     Решение — для "плотных" субмешей принудительно включаем
        //     glDepthMask(GL_TRUE), чтобы рука выглядела цельной.
        //
        //   КАК ПОДКРУТИТЬ ПОРОГ ВЕРШИН:
        //     Текущий порог: 3000 вершин.
        //     Субмеши с меньшим числом вершин = детали лица и малые части тела.
        //     Если руки всё ещё "стеклянные" → увеличить порог до 5000 или 8000.
        //     Если что-то лишнее стало непрозрачным → уменьшить до 2000.
        //
        //   КАК ПОДКРУТИТЬ СМЕЩЕНИЕ РТА:
        //     glPolygonOffset(-2.0f, -2.0f) — текущее значение.
        //     Если рот всё равно "тонет" в коже → изменить на (-3.0f, -3.0f).
        //     Если рот "плавает" перед лицом    → изменить на (-1.0f, -1.0f).
        // =====================================================================

        // Порог 3000: считаем субмеш "плотной деталью" (лицо, руки, малые части).
        // Увеличить если руки/тело всё ещё просвечивают насквозь.
        bool isSolidPart = (pass == 1 && sm.m_vertexCount < 3000);

        if (isSolidPart) {
            // Polygon Offset: "выталкиваем" деталь ближе к камере, чтобы
            // она перекрыла кожу без Z-fighting.
            glEnable(GL_POLYGON_OFFSET_FILL);
            glPolygonOffset(-2.0f, -2.0f);  // === РОТ/ГЛАЗА: -2.0f = надёжно над кожей
                                             // Мерцает? → -3.0f. Плавает? → -1.0f.

            // DepthMask GL_TRUE: деталь пишет в Z-буфер → рука выглядит
            // цельной, а не "стеклянной" при уменьшении общей прозрачности.
            glDepthMask(GL_TRUE);
        }
        ...
        if (isSolidPart) {
            ...
            // Возвращаем GL_FALSE для крупных прозрачных субмешей (волосы, одежда),
            // чтобы они продолжали корректно блендиться поверх всего.
            glDepthMask(GL_FALSE);
        }
    } // end submesh loop
